...
import { PostgresFreightRepository, VehicleRepository, type FreightRow } from '@tms/database';
...
function toFreight(row: FreightRow): Freight {
  return {
    ...
    destination: { city: row.destinationCity, state: row.destinationState, country: 'BR' },
    cargo: {
      description: row.cargoDescription,
      quantity: row.quantity,
      weightKg: Number(row.weightKg),
      volumeM3: row.volumeM3 === null ? undefined : Number(row.volumeM3),
      linearMeters: row.linearMeters === null ? undefined : Number(row.linearMeters),
      valueCents: row.customerPriceCents === null ? undefined : BigInt(row.customerPriceCents),
    },
    vehicleRequirement: {
      types: row.vehicleTypes as VehicleType[],
      bodies: row.bodyTypes as BodyType[],
      minimumFreeMeters: row.minimumFreeMeters === null ? undefined : Number(row.minimumFreeMeters),
      minimumCapacityKg: row.minimumCapacityKg === null ? undefined : Number(row.minimumCapacityKg),
    },
    driverPrice: row.driverPriceCents === null ? undefined : { amountCents: BigInt(row.driverPriceCents), currency: 'BRL' },
    customerPrice: row.customerPriceCents === null ? undefined : { amountCents: BigInt(row.customerPriceCents), currency: 'BRL' },
    createdAt: new Date(),
    updatedAt: new Date(),
  };
}
...
export class MatchingService {
  ...
  async rank(context: RequestContext, freightId: string): Promise<readonly MatchResult[]> {
