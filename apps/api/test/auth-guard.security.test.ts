...
async function token(overrides: Record<string, unknown> = {}) {
  return new SignJWT({ tenantId: TENANT_A, roles: ['admin'], permissions: ['*'], ...overrides })
...
test('AuthGuard rejects a forged tenant selection when membership does not exist', async () => {
  ...
  await assert.rejects(
    ...
    (error: unknown) => error instanceof Error && error.message === 'Active tenant membership required',
  );
});
...
test('AuthGuard rejects an inactive tenant membership', async () => {
  ...
  await assert.rejects(
    () => guard([{ userId: USER_ID, tenantId: TENANT_A, role: 'operator', permissions: [], active: false }]).canActivate(contextFor(request)),
    (error: unknown) => error instanceof Error && error.message === 'Active tenant membership required',
  );
});
