...
import TransportesPage from "./transportes-client";
...
export default async function ModulePage({params}:{params:Promise<{section:string}>}){const {section}=await params;if(section==="transportes")return <TransportesPage/>;const m=modules[section]??{title:"Módulo",eyebrow:"TMS",description:"Área operacional do TMS.",items:[]};return <main className="module-page"><Link href="/app" className="back-link">← Voltar ao dashboard</Link><span className="eyebrow">{m.eyebrow}</span><h1>{m.title}</h1><p>{m.description}</p><div className="module-grid">{m.items.map((x,i)=><article key={x}><span>0{i+1}</span><h2>{x}</h2><small>Área preparada para a próxima etapa operacional.</small></article>)}</div></main>}