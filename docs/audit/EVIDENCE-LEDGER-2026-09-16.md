...
|---|---|---|---|---|---|---|---|
| EV-001 | Controller | Canonical repository and branch | GitHub repository metadata; current `main` HEAD is `98ed9ba120a863ac4074adedc8c37df0872775ab` | E4 | COMPROVADO | P0 | Keep controller anchored to `main` |
...
| EV-009 | CI/CD | Current main quality chain is green | CI #881 / run `35302942154` on `f0a49d80...`; migration, runtime role/RLS/IAM, explicit Durable Jobs integration, format, lint, typecheck, test and build all passed | E4 | COMPROVADO | P1 | Preserve green baseline and continue blocker routing |
...
| EV-015 | Reconciliation | Controller baseline is current | `main` advanced to `98ed9ba...` through the latest Evidence Ledger update; Vercel production deployment is READY on `98ed9ba...` | E4 | RECONCILED | P1 | Use `8125a025...` as current repository baseline |
...
| EV-029 | CHAT 14 / Runtime | Current Vercel production deployment is reachable and `/health` returns HTTP 200 | Vercel project `tms-core-api`; latest READY production deployment `dpl_FkPbusf5xCmgaNUFKmVCUyPrcpYR`, commit `98ed9ba120a863ac4074adedc8c37df0872775ab`; deployment build completed successfully; `/health` returned `200 OK` with `{"status":"ok","service":"tms-api"}` on 2026-09-18 03:30:40 GMT | E4 | COMPROVADO | P1 | Authenticated runtime path still pending; Vercel environment values and live Auth0 Action/tenant claim are not exposed by current evidence |

| EV-030 | CHAT 23 / Frontend Auth | Current web app has no Auth0 SDK, auth routes, session provider, login/logout flow or access-token API integration | `apps/web/package.json` contains only Next/React dependencies; `layout.tsx` has no auth provider; `page.tsx` only calls `/health`; repository code search returned no frontend Auth0 login/session/getAccessToken integration | E1 | ABSENT / BLOCKED | P1 | Implement Frontend Auth Foundation using the supported Auth0 Next.js SDK, then prove Web → Auth0 → API with the production audience |
...
Repository code and CI prove the API OIDC validation contract, but the web application currently lacks an Auth0 client/session integration. Current evidence therefore cannot prove deployed issuer/audience values, the live Auth0 Action tenant claim, or a real Web → Auth0 → API authenticated request. Frontend Auth Foundation is the immediate corrective gate.
...
ACTIVE — current `main` is `98ed9ba120a863ac4074adedc8c37df0872775ab`. Vercel production latest deployment `dpl_FkPbusf5xCmgaNUFKmVCUyPrcpYR` is READY on that exact commit, and Railway `tms-worker` has a successful production deployment sourced from `alexoaraujo83/TMS` `main`. Infrastructure is therefore no longer an unverified deployment concept, but runtime activation remains blocked by zero configured tenants / disabled Durable Jobs and the intended service topology still needs reconciliation. Durable Jobs heartbeat/renewal, fail-closed webhook behavior, and the real PostgreSQL persistence → claim → handler → publication → finalization path remain CI-proven. Environment drift, backup/DR, runtime smoke, functional traceability, receiver-side webhook semantics and infrastructure topology remain open. CI #881 is green on current main. Frontend Auth Foundation remains the next implementation gate. Next route: CHAT 23 — Frontend Auth Foundation, then CHAT 14 Auth0 runtime reconciliation → Tenant/RBAC/RLS runtime smoke, without bypassing BLK-001/002/003/004/005/006/007.
