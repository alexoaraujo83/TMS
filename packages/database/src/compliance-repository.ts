...
export class ComplianceRepository {
    ...
    return withTransaction(
      ...
      async (client) => {
        const freight = await client.query(
          "select id from freights where tenant_id = $1 and id = $2 for update",
        ...
        const result = await client.query(
          `insert into compliance_checks (
             tenant_id, freight_id, assignment_id, check_type, provider,
             external_reference, metadata, expires_at
    ...
    return withTransaction(this.pool, { tenantId }, async (client) => {
      const result = await client.query(
        `select ${COMPLIANCE_COLUMNS}
         ...
         where tenant_id = $1 ${freightId ? "and freight_id = $2" : ""}
         order by created_at desc`,
        freightId ? [tenantId, freightId] : [tenantId],
      );
      return result.rows;
    });
  }
  ...
  async transitionCheck(
    ...
    return withTransaction(this.pool, { tenantId }, async (client) => {
      ...
      if (row.status !== expected) {
        throw new Error(
          `Compliance status ${row.status} does not match expected status ${expected}`,
        );
      }
      if (!isAllowedComplianceTransition(expected, next)) {
        throw new Error(
          `Compliance check cannot transition from ${expected} to ${next}`,
        );
      }
      ...
      async (client) => {
        ...
        const result = await client.query(
          `insert into gr_requests (
             tenant_id, freight_id, assignment_id, provider, protocol,
             external_reference, metadata, expires_at
           ) values ($1,$2,$3,$4,$5,$6,$7::jsonb,$8)
           returning ${GR_COLUMNS}`,
          [
            input.tenantId,
            input.freightId,
            input.assignmentId ?? null,
  ...
  async transitionGr(
    ...
    return withTransaction(this.pool, { tenantId }, async (client) => {
      ...
      if (row.status !== expected) {
        throw new Error(
          `GR status ${row.status} does not match expected status ${expected}`,
        );
      }
      if (!isAllowedGrTransition(expected, next)) {
        throw new Error(
          `GR request cannot transition from ${expected} to ${next}`,
        );
      }
      ...
      const result = await client.query(
        `update gr_requests
         set status = $3,
             submitted_at = case
               when $3 = 'submitted' and submitted_at is null then now()
               else submitted_at
             end,
...
function isAllowedComplianceTransition(
...
function isAllowedGrTransition(from: GrStatus, to: GrStatus) {
  return (
    ...
    (from === "submitted" &&
      ["approved", "rejected", "expired", "cancelled"].includes(to)) ||
    (from === "approved" && to === "expired")
  );
}
