      ...
      const result = await client.query(
        `insert into outbox_events
         ...
         returning id, tenant_id, aggregate_type, aggregate_id, event_type, payload,
           status, attempts, available_at, lease_token, published_at, last_error,
      ...
      const result = await client.query(
        `with claimed as (
         ...
         returning event.id, event.tenant_id, event.aggregate_type,
           ...
           event.attempts, event.available_at, event.lease_token, event.published_at,
      ...
      const result = await client.query(
        `update outbox_events
         ...
         returning id, tenant_id, aggregate_type, aggregate_id, event_type, payload,
           status, attempts, available_at, lease_token, published_at, last_error,
           created_at, updated_at`,
        [tenantId, id, maxAttempts, retryAt, error.slice(0, 4000), leaseToken],
      );
