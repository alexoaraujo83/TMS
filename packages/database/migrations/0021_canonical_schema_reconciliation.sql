...
-- This migration also repairs the legacy zero-data shape found during audit.

-- Temporary compatibility columns allow one migration to handle both shapes.
...
-- Remove legacy compatibility columns after canonical values are populated.
alter table compliance_checks
...
-- Keep the final trip lifecycle invariant deterministic and idempotent.
alter table trips
